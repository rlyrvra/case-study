...
<table class="table table-bordered table-hover">
  ...
  <tbody>
    <?php if (!empty($allowances)): ?>
      <?php $i = ($offset + 1); foreach ($allowances as $row): ?>
    ...
    <?php else: ?>
      <tr>
        <td colspan="7" style="text-align: center; padding: 20px; color: #888;">No data available</td> 
      </tr>
    <?php endif; ?>
  </tbody>
  ...
  <nav aria-label="Page navigation" class="d-flex justify-content-center">
    <ul class="pagination pagination-lg">
      ...
      <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
        <a class="page-link" onclick="fetchAllAllowances('prev')" aria-label="Previous">
      ...
      <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        ...
        <li class="page-item <?= $i === $page ? 'active' : '' ?>">
          <a class="page-link" onclick="fetchAllAllowances(<?php echo $i ?>)" ><?= $i ?></a>
        </li>
      <?php endfor; ?>
      <!-- Next Button -->
      <li class="page-item <?= $page >= $totalPages ? 'disabled' : '' ?>">
        <a class="page-link" onclick="fetchAllAllowances('next')" aria-label="Next">
